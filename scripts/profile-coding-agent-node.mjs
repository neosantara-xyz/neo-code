...
const agentDirEnvName = "NAI_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "NAI_CODE_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific NAI_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force NAI_CODE_OFFLINE=1 / NAI_CODE_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.NAI_CODE_OFFLINE = "1";
		env.NAI_CODE_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "nai-startup-benchmark-")) : undefined;
