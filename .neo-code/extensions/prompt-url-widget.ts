import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@neosantara-xyz/code";
...
async function fetchGhMetadata(
	...
	try {
		const result = await neoApi.exec("gh", args);
...
export default function promptUrlWidgetExtension(neoApi: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, title?: string) => {
		...
		const currentName = neoApi.getSessionName()?.trim();
		if (!currentName) {
			neoApi.setSessionName(desiredName);
			return;
		}
	...
	neoApi.on("before_agent_start", async (event, ctx) => {
		if (!ctx.hasUI) return;
		...
		void fetchGhMetadata(neoApi, match.kind, match.url).then((meta) => {
	...
	neoApi.on("session_switch", async (_event, ctx) => {
	...
	neoApi.on("session_start", async (_event, ctx) => {
		rebuildFromSession(ctx);
	});
}
