...
const agentDirEnvName = "NEO_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "NEO_CODE_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific NEO_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force NEO_CODE_OFFLINE=1 / NEO_CODE_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.NEO_CODE_OFFLINE = "1";
		env.NEO_CODE_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "neo-startup-benchmark-")) : undefined;
