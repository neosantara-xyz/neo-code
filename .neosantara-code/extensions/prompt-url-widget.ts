import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@neosantara/code";
...
async function fetchGhMetadata(
	...
	try {
		const result = await naiApi.exec("gh", args);
...
export default function promptUrlWidgetExtension(naiAnaiApi: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, title?: string) => {
		...
		const currentName = naiApi.getSessionName()?.trim();
		if (!currentName) {
			naiApi.setSessionName(desiredName);
			return;
		}
	...
	naiApi.on("before_agent_start", async (event, ctx) => {
		if (!ctx.hasUI) return;
		...
		void fetchGhMetadata(naiApi, match.kind, match.url).then((meta) => {
	...
	naiApi.on("session_switch", async (_event, ctx) => {
	...
	naiApi.on("session_start", async (_event, ctx) => {
		rebuildFromSession(ctx);
	});
}
