...
export async function resolveModelScope(patterns: string[], modelRegistry: ModelRegistry): Promise<ScopedModel[]> {
	...
	for (const pattern of patterns) {
		...
		if (pattern.includes("*") || pattern.includes("?") || pattern.includes("[")) {
			...
			// This allows model globs to match without requiring a provider prefix
...
export function resolveCliModel(options: {
	...
	// This handles models whose IDs naturally contain slashes.
	if (!provider) {
	...
	// This handles slash-delimited model IDs like "vendor/model:variant" where "vendor"
